...
function showPage(pageId) {
    ...
    // Hide sidebar on public pages (landing, login, register) or during a quiz.
    // When a quiz is running we want the user to focus and not escape via navigation.
    const sidebar = document.getElementById('sidebar');
    if (pageId === 'frontPage' || pageId === 'loginPage' || pageId === 'registerPage' || pageId === 'quizPage') {
...
function updateSidebarMenu(pageId) {
    ...
    // nav order now: Home(0), Start(1), Stats(2), About(3), Contact(4), Logout(5)
    if (pageId === 'homePage') {
...
function handleLogin(event) {
    ...
    // allow admin credentials without normal constraints
    if (email === 'admin' && password === 'admin') {
        // skip further checks
    } else {
        // password length enforcement for regular users
    ...
    if (email === 'admin' && password === 'admin') {
        ...
        // ensure an admin user exists in the users list for visibility
        const users = JSON.parse(localStorage.getItem('users') || '[]');
        if (!users.find(u => u.email === 'admin')) {
            users.push({ id: 'admin', name: 'Administrator', email: 'admin', password: 'admin', quizzes: [] });
            localStorage.setItem('users', JSON.stringify(users));
        }
...
function handleLogout() {
    if (confirm('Are you sure you want to logout?')) {
        ...
        // If on admin page (no sidebar or showFrontPage), just redirect
...
function setSelectedUser(idx) {
    // toggle off if clicking the currently selected row
    if (idx === selectedUserIndex) {
        idx = null;
    }
    selectedUserIndex = idx;
    document.querySelectorAll('#userTable tr.user-row').forEach(row => {
    ...
    document.getElementById('editUserBtn').disabled = idx === null;
    document.getElementById('deleteUserBtn').disabled = idx === null;
}
...
function loadAdminUsers() {
    ...
    table.querySelectorAll('tr.user-row').forEach(r => r.remove());

    users.forEach((u, idx) => {
        // do not show the built-in admin account
        if (u.email === 'admin' && u.password === 'admin') return;
...
function loadContactMessages() {
    ...
    table.querySelectorAll('tr').forEach(r => {
        if (r.parentElement.tagName === 'TBODY') r.remove();
    });
    const tbody = table.querySelector('tbody');
    if (msgs.length === 0) {
        const tr = document.createElement('tr');
        tr.innerHTML = '<td colspan="5" style="text-align:center; color:#666;">No feedback messages.</td>';
        tbody.appendChild(tr);
    } else {
        msgs.forEach(m => {
            const tr = document.createElement('tr');
            tr.innerHTML = `<td>${m.name}</td><td>${m.email}</td><td>${m.phone || ''}</td><td>${m.message}</td><td>${new Date(m.date).toLocaleString()}</td>`;
            tbody.appendChild(tr);
        });
    }
    const clearBtn = document.getElementById('clearMessagesBtn');
    if (clearBtn) clearBtn.disabled = msgs.length === 0;
}
...
function deleteUser(index) {
    ...
    if (index >= 0 && index < users.length) {
        const u = users[index];
        // protect admin account from removal
...
function editUser(index) {
    ...
    // prevent admin account being modified
...
function saveEditedUser() {
    ...
    const users = JSON.parse(localStorage.getItem('users')) || [];
    if (editingUserIndex < 0 || editingUserIndex >= users.length) return;
    ...
    users[editingUserIndex] = { ...users[editingUserIndex], name, email, password };
    localStorage.setItem('users', JSON.stringify(users));
...
function ensureAdmin() {
    if (localStorage.getItem('currentAdmin') !== 'true') {
...
function initPage() {
    if (document.getElementById('userTable')) {
        ...
        if (saveBtn) saveBtn.addEventListener('submit', e => {
            e.preventDefault();
            saveEditedUser();
        });
    ...
    if (current && document.getElementById('dashboardName')) {
        updateHomeStats();
    }
}
...
function startQuiz(level, gameType) {
    // bail out if we don't have a quiz page to render to
    if (!document.getElementById('quizPage')) {
        alert('Quiz functionality is not available on this page.');
        return;
    }
    ...
    // Guard: if still no valid level, redirect back to level selection (if present)
    if (!level) {
        alert('Please select a difficulty level first!');
        if (document.getElementById('quizSelectionPage')) showPage('quizSelectionPage');
        return;
    }
    ...
    // ensure next button text and visibility reset
...
function displayQuestion() {
    ...
    document.querySelectorAll('.answer-btn, .true-false-btn').forEach(btn => {
...
function selectAnswer(answer) {
    const question = currentQuiz.questions[currentQuiz.currentQuestion];
    // allow changing until quiz ends; no locking
...
function nextQuestion() {
    ...
    if (currentQuiz.currentQuestion < maxQuestions) {
        displayQuestion();
        // if we've just moved to the last question, change the button text
        if (currentQuiz.currentQuestion === maxQuestions - 1) {
...
function endQuiz() {
    ...
    // compute score for MC and T/F quizzes (matching score already tracked earlier)
    if (currentQuiz.gameType === 'multipleChoice' || currentQuiz.gameType === 'trueOrFalse') {
        currentQuiz.score = 0;
        currentQuiz.questions.forEach((question, idx) => {
            const userAns = currentQuiz.selectedAnswers[idx];
            if (userAns !== undefined && userAns !== null) {
                // for MC the answers are indices, for T/F booleans
                if (userAns === question.correct) {
                    currentQuiz.score++;
                }
            }
        });
    ...
    document.getElementById('correctCount').textContent = currentQuiz.score + '/' + totalQuestions;
...
function renderHomeDashboard() {
    ...
    if (lb) {
        ...
        // filter out the built-in admin account
        users = users.filter(u => !(u.email === 'admin' && u.password === 'admin'));
        if (users.length === 0) {
